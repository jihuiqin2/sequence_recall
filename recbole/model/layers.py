...
class FreqAttention(nn.Module):
    ...
    def time_delay_agg_training(self, values, corr):
        ...
        # find top k
        top_k = int(self.factor * math.log(length))  # 相关中排名前 top_k=3 的值
        mean_value = torch.mean(torch.mean(corr, dim=1), dim=1)  # [256,50]
        index = torch.topk(torch.mean(mean_value, dim=0), top_k, dim=-1)[
            1]  # 根据上一步得到的平均值，选择排名前 top_k 的索引，这些索引表示在自相关中要考虑的最重要的位置。
        weights = torch.stack([mean_value[:, index[i]] for i in range(top_k)], dim=-1)  # [256,3]
        # update corr
        ...
        for i in range(top_k):
            pattern = torch.roll(tmp_values, -int(index[i]), -1)  # [256,2,23,50]
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        values = self.transpose_for_scores(mixed_value_layer)  # [256,2,50,32]
        ...
        # period-based dependencies  [256,2,26,32] 它将一个实数信号从时间域转换到频率域
        q_fft = torch.fft.rfft(queries.contiguous(), dim=2)
        k_fft = torch.fft.rfft(keys.contiguous(), dim=2)
        v_fft = torch.fft.rfft(values.contiguous(), dim=2)

        # 装到采样的空盒子里
        q_fft_box = torch.zeros(B, H, len(self.q_index), E, device=q_fft.device, dtype=torch.cfloat)
        # print(box_xq_ft.shape, xq_ft.shape)
        for i, j in enumerate(self.q_index):
            q_fft_box[:, :, :, i] = q_fft[:, :, :, j]

        k_fft_box = torch.zeros(B, H, len(self.k_index), E, device=k_fft.device, dtype=torch.cfloat)
        for i, j in enumerate(self.q_index):
            k_fft_box[:, :, :, i] = k_fft[:, :, :, j]

        v_fft_box = torch.zeros(B, H, len(self.k_index), E, device=v_fft.device, dtype=torch.cfloat)
        for i, j in enumerate(self.q_index):
            v_fft_box[:, :, :, i] = v_fft[:, :, :, j]

        attention_score = torch.matmul(q_fft_box, k_fft_box.transpose(-1, -2))  # [256,2,26,26]
        # attn_scores_real = torch.einsum('bhqf,bhkf->bhqk', q_fft_box, k_fft_box)
        # attn_scores_imag = torch.einsum('bhqf,bhkf->bhqk', q_fft_box, k_fft_box * -1.0)
        # attention_score = torch.complex(attn_scores_real, attn_scores_imag)
        attention_score /= k_fft.size(-1) ** 0.5  # Normalize

        context_layer = torch.matmul(attention_score, v_fft_box)  # [256,2,26,32]

        corr = torch.fft.irfft(context_layer, dim=2)  # 逆变换从频率域转换回时间域[256,2,26,32]

        # print(V.shape) [256,50,64]
        V = corr.permute(0, 2, 1, 3).contiguous()
...
class HybridAttention(nn.Module):
    ...
    def time_delay_agg_training(self, values, corr):
        ...
        mean_value = torch.mean(torch.mean(corr, dim=1), dim=1)  # [b,50]
        index = torch.topk(torch.mean(mean_value, dim=0), top_k, dim=-1)[1]
        weights = torch.stack([mean_value[:, index[i]] for i in range(top_k)], dim=-1)  # [b,3]
        # update corr
        tmp_corr = torch.softmax(weights, dim=-1)
        # aggregation
        tmp_values = values
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        if L > S:
            ...
            # period-based dependencies  [256,2,32,26]
        ...
        corr = torch.fft.irfft(box_res, dim=-1)  # [256,2,32,50]
...
class FEABlock(nn.Module):
    ...
    def __init__(
        ...
        self.hybrid_attention = FreqAttention(
...
class FEAEncoder(nn.Module):
    ...
    def __init__(
        ...
        for n in range(self.n_layers):
            self.layer_ramp = FEABlock(n_heads, hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob,
                                       hidden_act, layer_norm_eps, n, config)
