...
class Trainer(AbstractTrainer):
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            ...
            #  todo add noise start ----------------------
            ration_len = np.floor(interaction['item_length'] * self.config['noisy']).to(torch.int).tolist()
            for k in range(len(ration_len)):
                origin_mask_length = interaction['item_length'][k].tolist()
                item = [random.randint(1, self.tot_item_num - 1) for _ in range(ration_len[k])]
                if len(item) == 0:
                    continue
                item = torch.tensor(item)
                index = torch.tensor([random.randint(0, origin_mask_length - 1) for _ in range(ration_len[k])])
                interaction['item_id_list'][k][index] = item
            #  todo add noise end ----------------------
