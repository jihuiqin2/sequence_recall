...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False, phase='train'):
        ...
        elif config['SSL_AUG'] == 'CLF4SRec' and self.phase == 'train':
            self.same_target_index = dataset.same_target_index
    ...
    def data_preprocess(self):
        ...
        if self.config['model'] == 'FEARec' or 'CLF4SRec' or 'CF4SRec':
            # 数据集之间不一样
            if self.config['dataset'] == 'yelp2':
    ...
    def _shuffle(self):
        if self.real_time:
            ...
            if self.config['SSL_AUG'] == 'CLF4SRec' and self.phase == 'train':
                self.same_target_index = self.same_target_index[self.pre_processed_data.index]
    ...
    def _get_processed_data(self, index):
        ...
        if self.config['SSL_AUG'] == 'CL4SRec' and self.phase == 'train':
            ...
            self.myrec_aug(cur_data)  # todo step2: sparsity
    ...
    def myrec_aug(self, cur_data):
        ...
        def item_mask(seq, length, gamma=0.3):
            ...
            masked_item_seq = seq[:]
            masked_item_seq[mask_index] = 0  # todo token 0 has been used for semantic masking
        ...
        cur_data.update(Interaction({'sem_aug': torch.stack(aug_seq_r), 'sem_aug_lengths': torch.stack(aug_len_r)}))
